...
function mirrorToLocalStorage() {
  localStorage.setItem('items', JSON.stringify(items));   //mirror to local storage, local storage is text only and we need to convert the object to string using JSON
}


//when we load the page restore from local storage on page load
function restoreFromLocalStorage() {
  const lsItems = JSON.parse(localStorage.getItem('items'));    //pull the item from local storage. use json.parse to get back the array
  if (lsItems.length) {
    items = lsItems;
    // fire event
